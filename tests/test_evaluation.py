...
def test_slice_evaluation_window_without_eval_start_returns_copy():
    equity_df = pd.DataFrame(
        {"Equity": [100, 105]},
        index=pd.to_datetime(["2024-01-03", "2024-01-04"]),
    )
    eval_equity, eval_trades = slice_evaluation_window(equity_df, pd.DataFrame())
    assert eval_equity["Equity"].tolist() == [100, 105]
    assert eval_trades.empty
